...
export default function FounderActions({ founder }: { founder: Founder }) {
  ...
  const [twitter, setTwitter] = useState(founder.twitter ?? "");
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch(
        `/api/founders?id=${encodeURIComponent(founder.id)}`,
        {
          method: "PATCH",
      ...
      <div className="mt-3 flex items-center gap-3">
        ...
        {confirming ? (
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setConfirming(false)} className="text-xs text-[var(--muted)] hover:text-[var(--text)]">
              No
            </button>
          </div>
        ) : (
          <button onClick={() => setConfirming(true)} className="text-xs text-[var(--muted)] hover:text-red-400 transition-colors">
            Delete
          </button>
        )}
  ...
  return (
    <div className="mt-6 rounded-lg border border-[var(--accent)]/30 bg-[var(--surface)] p-5">
      <h2 className="mb-4 text-sm font-medium text-[var(--text)]">Edit founder</h2>
      <div className="space-y-3">
        ...
        <div className="flex justify-end gap-3 pt-1">
          <button onClick={() => { setEditing(false); setName(founder.name); setTwitter(founder.twitter ?? ""); setLinkedin(founder.linkedin ?? ""); setNotes(founder.notes ?? ""); }} className="rounded-md px-4 py-2 text-sm text-[var(--muted)] hover:text-[var(--text)]">
            Cancel
          </button>
          <button onClick={handleSave} disabled={saving || !name.trim()} className="rounded-md bg-[var(--accent)] px-4 py-2 text-sm font-medium text-black transition-opacity hover:opacity-90 disabled:opacity-40">
            {saving ? "Saving…" : "Save"}
          </button>
        </div>
      </div>
